...
const memorySource = computed(() => props.filteredMemories)
const {
  page: memoryPage,
  pageSize: memoryPageSize,
  total: memoryTotal,
  paginatedItems: paginatedMemories,
  setPage: setMemoryPage,
  setPageSize: setMemoryPageSize,
  resetPage: resetMemoryPage
} = usePagination(memorySource, 5)

watch(() => [props.memorySearch, props.activeMemoryType], resetMemoryPage)
    ...
    <section class="memory-content">
      ...
      <section class="memory-main">
        <div class="memory-list">
          <article v-for="item in paginatedMemories" :key="item.title" class="memory-item panel" :class="{ active: item.active }" @click="emit('selectMemory', item)">
          ...
          <PaginationBar
            ...
            :total="memoryTotal"
            item-label="条记忆"
            @update:page="setMemoryPage"
            @update:page-size="setMemoryPageSize"
          />
        </div>
