...
const agentSource = computed(() => props.filteredAgents)
const {
  page: agentPage,
  ...
  setPageSize: setAgentPageSize,
  resetPage: resetAgentPage
} = usePagination(agentSource, 6)

watch(() => props.agentSearch, resetAgentPage)
    ...
    <section class="agents-dashboard">
      ...
      <section class="agents-list panel">
        ...
        <div class="agent-card-grid">
          <article v-for="agent in paginatedAgents" :key="agent.name" class="agent-manage-card" :class="{ featured: agent.name === selectedAgentName }" @click="emit('selectAgent', agent)">
        ...
        <PaginationBar
          ...
          @update:page-size="setAgentPageSize"
        />
      </section>
